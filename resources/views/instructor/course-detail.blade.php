...
@section('content')
    <div class="p-8">
        ...
        <div class="mb-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="flex gap-3">
                ...
                <button onclick="scrollToModulKursus()"
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            ...
            @else
                <div class="flex flex-col sm:flex-row gap-3 mb-4">
                    <input id="insParticipantSearch" type="text" placeholder="Cari peserta..."
                        class="w-full sm:w-1/3 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                    <select id="insParticipantFilter"
                        class="w-full sm:w-40 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        <option value="all">Semua status</option>
                        <option value="passed">Lulus</option>
                        <option value="failed">Tidak lulus</option>
                        <option value="none">Belum ada nilai</option>
                    </select>
                    <select id="insParticipantSort"
                        class="w-full sm:w-48 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <table class="min-w-full" id="insParticipantTable">
                        <thead class="bg-gray-50 border-b sticky top-0 z-10">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase">Nilai Terbaik
                                </th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase">Terakhir Submit
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @foreach($participantProgress as $participant)
                                ...
                                @endphp
                                <tr class="hover:bg-gray-50 ins-participant-row" data-name="{{ strtolower($user->name ?? '') }}"
                                    data-email="{{ strtolower($user->email ?? '') }}" data-progress="{{ $progressVal }}"
                                    data-score="{{ $bestPct ?? -1 }}" data-last="{{ $lastSubmitTs }}"
                                        ...
                                        @if(!is_null($bestPct))
                                            <div class="flex flex-col items-center gap-1">
                                                <div class="text-sm font-semibold text-gray-900">
                                                    {{ number_format($bestPct, 1) }}% <span class="text-xs text-gray-500">(attempt
                                                        {{ $participant['best_attempt'] ?? '-' }})</span>
                                                </div>
                                                <div class="text-xs text-gray-500">Skor:
                                                    {{ number_format($participant['best_score'] ?? 0, 1) }}
                                                </div>
                                                @if($participant['is_passed'] === true)
                                                    <span
                                                        class="px-2 py-1 bg-emerald-50 text-emerald-700 rounded-full text-[11px] font-semibold">Lulus</span>
                                                @elseif($participant['is_passed'] === false)
                                                    <span
                                                        class="px-2 py-1 bg-red-50 text-red-700 rounded-full text-[11px] font-semibold">Tidak
                                                        lulus</span>
                                                @else
                                                    <span
                                                        class="px-2 py-1 bg-gray-100 text-gray-600 rounded-full text-[11px] font-semibold">Belum
                                                        ada nilai</span>
                                                @endif
                                            </div>
                                        @else
        ...
        <!-- Modul Kursus -->
        <div id="modulKursusSection" class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="p-6 border-b border-gray-200 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                ...
                <form action="{{ route('instructor.courses.modules.destroy-all', $course) }}" 
                ...
                <form action="{{ route('instructor.courses.materials.destroy-all', $course) }}" 
                                        ...
                                        <div class="flex gap-2">
                                            ...
                                            @if(in_array($course->metode, ['offline', 'hybrid']))
                                                <button onclick="openMaterialModal({{ $section->id }}, 'class_session')"
                                                    ...
                                                    <i class="fas fa-users"></i> Sesi Tatap Muka
                                                </button>
                                            @endif
                                        </div>
                                    </div>
                                                        ...
                                                        <a href="{{ route('instructor.courses.materials.preview', [$course, $material]) }}"
                                                            ...
                                                            <p class="text-xs text-gray-500">
                                                                @if($material->type === 'class_session')
                                                                    <span
                                                                        class="text-orange-600 font-medium">{{ ucfirst($material->session_type ?? 'offline') }}</span>
                                                                    ...
                                                                    @if($material->session_location)
                                                                        ...
                                                                        {{ Str::limit($material->session_location, 20) }}
                                                                    @endif
                                                                @else
    ...
    <div id="materialModal" class="hidden fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto shadow-2xl relative">
            ...
            <div class="relative z-10 mb-4">
                <h3 class="text-xl font-bold text-gray-800 flex items-center gap-2" id="materialModalTitle">
                    <i class="fas fa-file-upload text-blue-600" id="materialModalIcon"></i>
            ...
            <form action="{{ route('instructor.courses.materials.store', $course) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4 relative z-10">
                    <div class="flex items-start gap-3 bg-blue-50 border border-blue-100 text-blue-800 rounded-xl px-3 py-2 text-sm mb-3"
                        ...
                        <div id="materialAlertText">Unggah video (.mp4/.mov) atau PDF sesuai tipe materi. Untuk teks, isi
                            konten tanpa unggah file.</div>
                    </div>
                ...
                <div id="classSessionFields" style="display: none;">
                    <div class="mb-4 relative z-10 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Sesi *</label>
                            <input type="date" name="session_date"
                                class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500 shadow-sm">
                        </div>
                        <div>
                    ...
                    <div class="mb-4 relative z-10">
                        ...
                        <input type="text" name="session_location" placeholder="Contoh: Ruang Lab 301, Gedung A"
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500 shadow-sm">
                    </div>
                    <div class="mb-4 relative z-10">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Link Meeting (untuk sesi online)</label>
                        <input type="url" name="session_meeting_link" placeholder="https://zoom.us/j/..."
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500 shadow-sm">
                    </div>
                </div>
    ...
    <div id="quizModal" class="hidden fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl max-w-xl w-full p-6 max-h-[90vh] overflow-y-auto shadow-2xl relative">
            ...
            <form action="{{ route('instructor.courses.quizzes.store', $course) }}" method="POST">
                ...
                <div class="mb-4 relative z-10">
                    <div
                        class="flex items-start gap-3 bg-yellow-50 border border-yellow-100 text-yellow-800 rounded-xl px-3 py-2 text-sm mb-3">
                ...
                <div class="mb-4 relative z-10">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-yellow-500 shadow-sm"></textarea>
                </div>
                ...
                <div class="grid md:grid-cols-1 gap-4 mb-4 relative z-10">
                    <div>
                        ...
                        <input type="number" name="time_limit" value="30" min="1"
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg shadow-sm" placeholder="Contoh: 30">
                ...
                <div class="flex gap-3 justify-end relative z-10">
                    ...
                    <button type="submit"
                        class="px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 shadow-md">
                        Simpan Quiz
                    </button>
                </div>
            </form>
            ...
            function toggleSection(sectionId) {
                        ...
                        if (content && chevron) {
                            ...
                            chevron.classList.toggle('rotate-90');
                                        ...
                                        // Fungsi untuk scroll ke section Modul Kursus saja (tanpa membuka modal)
                                        function scrollToModulKursus() {
                                            const modulKursusSection = document.getElementById('modulKursusSection');
                                            if (modulKursusSection) {
                                                modulKursusSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
                                            }
                                        }
                                        ...
                                        function openMaterialModal(sectionId = null, type = 'video') {
                                            ...
                                            const modalTitleText = document.getElementById('materialModalTitleText');
                                            const modalIcon = document.getElementById('materialModalIcon');
                                            const submitBtn = document.getElementById('materialSubmitBtn');
                                            const classSessionFields = document.getElementById('classSessionFields');
                                            ...
                                            if (type === 'video') {
                                                modalTitleText.textContent = 'Tambah Video';
                                                modalIcon.className = 'fas fa-video text-blue-600';
                                                alertBox.className = 'flex items-start gap-3 bg-blue-50 border border-blue-100 text-blue-800 rounded-xl px-3 py-2 text-sm mb-3';
                                                ...
                                                submitBtn.className = 'px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-md';
                                                submitBtn.textContent = 'Simpan Video';
                                            } else if (type === 'pdf') {
                                                modalTitleText.textContent = 'Tambah PDF';
                                                modalIcon.className = 'fas fa-file-pdf text-red-600';
                                                alertBox.className = 'flex items-start gap-3 bg-red-50 border border-red-100 text-red-800 rounded-xl px-3 py-2 text-sm mb-3';
                                                alertText.textContent = 'Unggah file PDF. Maksimal ukuran file: 100 MB.';
                                                fileInput.accept = '.pdf';
                                                document.getElementById('fileUploadSection').style.display = 'block';
                                                submitBtn.className = 'px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 shadow-md';
                                                submitBtn.textContent = 'Simpan PDF';
                                            } else if (type === 'text') {
                                                modalTitleText.textContent = 'Tambah Teks/Bacaan';
                                                modalIcon.className = 'fas fa-align-left text-green-600';
                                                alertBox.className = 'flex items-start gap-3 bg-green-50 border border-green-100 text-green-800 rounded-xl px-3 py-2 text-sm mb-3';
                                                alertText.textContent = 'Buat konten teks langsung di editor. Tidak perlu mengunggah file.';
                                                document.getElementById('contentSection').style.display = 'block';
                                                submitBtn.className = 'px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 shadow-md';
                                                submitBtn.textContent = 'Simpan Teks';
                                            } else if (type === 'class_session') {
                                                modalTitleText.textContent = 'Tambah Sesi Tatap Muka';
                                                modalIcon.className = 'fas fa-users text-orange-600';
                                                alertBox.className = 'flex items-start gap-3 bg-orange-50 border border-orange-100 text-orange-800 rounded-xl px-3 py-2 text-sm mb-3';
                                                alertText.textContent = 'Jadwalkan sesi tatap muka (offline) atau virtual (online) untuk peserta kursus.';
                                                classSessionFields.style.display = 'block';
                                                submitBtn.className = 'px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 shadow-md';
                                                submitBtn.textContent = 'Simpan Sesi';
                                            } else {
                                                modalTitleText.textContent = 'Tambah Materi';
                                                modalIcon.className = 'fas fa-file-upload text-blue-600';
                                                alertBox.className = 'flex items-start gap-3 bg-blue-50 border border-blue-100 text-blue-800 rounded-xl px-3 py-2 text-sm mb-3';
