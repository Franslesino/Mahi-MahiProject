...
<div class="p-8">
    <!-- Header -->
    <div class="mb-6">
        <a href="{{ route('instructor.courses.final-quiz.edit', $kursus->id) }}" class="text-blue-600 hover:text-blue-700 mb-2 inline-flex items-center gap-2">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="p-6">
            <form action="{{ route('instructor.courses.final-quiz.store-quiz', $kursus->id) }}" method="POST" id="createQuizForm">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="number" id="kesempatan_mengerjakan" name="kesempatan_mengerjakan" 
                               value="{{ old('kesempatan_mengerjakan', 3) }}" required
                               min="1" max="10"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 @error('kesempatan_mengerjakan') border-red-500 @enderror">
                        <small class="text-gray-600 text-sm">1-10 kali</small>
                        @error('kesempatan_mengerjakan')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="passing_grade" name="passing_grade" 
                               value="{{ old('passing_grade', 70) }}" required
                               min="0" max="100" step="0.01"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 @error('passing_grade') border-red-500 @enderror">
                ...
                <div class="mb-6">
                    ...
                    @if($questionBanks->count() === 0)
                        <div class="mt-4 bg-yellow-50 border-2 border-yellow-200 text-yellow-700 px-6 py-4 rounded-lg flex items-center gap-3">
                            ...
                            <a href="{{ route('instructor.question-banks.create') }}" target="_blank" class="text-blue-600 hover:underline font-semibold">Buat Question Bank baru</a>
                        </div>
                    @endif
                </div>
                            ...
                            <div class="flex-1 overflow-y-auto pr-2 space-y-3">
                                @foreach($questionBanks as $bank)
                                    <div class="border-2 border-gray-200 rounded-xl bg-white shadow-sm hover:shadow-md transition-shadow">
                                        <div class="flex items-center gap-3 p-4 cursor-pointer hover:bg-gray-50 transition rounded-t-xl" onclick="toggleBankModal('modal-bank-{{ $bank->id }}')">
                                            <i class="fas fa-chevron-right transition-transform duration-200 text-gray-500" id="icon-modal-bank-{{ $bank->id }}"></i>
                                            <i class="fas fa-folder text-purple-600 text-lg"></i>
                                            <h4 class="font-bold text-gray-800 flex-1">{{ $bank->title }}</h4>
                                            <span class="px-3 py-1 bg-purple-100 text-purple-700 text-sm font-semibold rounded-full">
                                                {{ $bank->questions->count() }} soal
                                            </span>
                                        </div>

                                        <div id="modal-bank-{{ $bank->id }}" class="modal-bank-content border-t border-gray-200 p-4 space-y-2.5 hidden bg-gray-50">
