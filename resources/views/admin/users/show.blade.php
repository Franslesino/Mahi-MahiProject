            ...
            <div class="flex items-center gap-2">
                ...
                @if($user->id !== auth()->id())
                    <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline"
                        data-confirm-title="Hapus pengguna?" data-confirm="Yakin ingin menghapus pengguna ini?">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <div class="text-center mb-6">
                        ...
                        @else
                            <div
                                class="w-32 h-32 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4">
                        ...
                        @if($user->role === 'admin')
                            <span
                                class="px-3 py-1 text-sm font-semibold rounded-full bg-purple-100 text-purple-700">Admin</span>
                        @elseif($user->role === 'instructor')
                            <span
                                class="px-3 py-1 text-sm font-semibold rounded-full bg-blue-100 text-blue-700">Instruktur</span>
                        @else
                        ...
                        @if($user->dob)
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-gray-800">{{ \Carbon\Carbon::parse($user->dob)->format('d M Y') }}
                                    </p>
                                </div>
                            </div>
                        @endif
            ...
            <div class="lg:col-span-2">
                                            ...
                                            <div class="flex-1">
                                                <div class="flex items-center gap-3 mb-2">
                                                    ...
                                                    @if($course->status_diterbitkan)
                                                        <span
                                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-700">Diterbitkan</span>
                                                    @else
                                                        <span
                                                ...
                                                <div class="flex items-center gap-4 text-sm text-gray-500">
                                                    ...
                                                    <span><i class="fas fa-users mr-1"></i>{{ $course->enrollments_count }}
                                                        peserta</span>
                                                    <span><i class="fas fa-money-bill-wave mr-1"></i>Rp
                ...
                @elseif($user->role === 'student' || $user->role === 'user')
                            ...
                            <div class="divide-y divide-gray-200">
                                @foreach($user->enrollments as $enrollment)
                                    <div class="p-6 hover:bg-gray-50">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <h4 class="text-lg font-semibold text-gray-800 mb-2">{{ $enrollment->kursus->judul }}
                                                </h4>
                                                <div class="flex items-center gap-4 text-sm text-gray-500 mb-2">
                                                    <span><i
                                                            class="fas fa-user mr-1"></i>{{ $enrollment->kursus->instructor->name ?? 'Tidak ada instruktur' }}</span>
                                                    <span><i class="fas fa-calendar mr-1"></i>Terdaftar:
                                                ...
                                                @if(in_array($status, ['completed', 'selesai']))
                                                    <span
                                                        class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-700">Selesai</span>
                                                @elseif(in_array($status, ['active', 'paid', 'approved', 'enrolled']))
                                                    <span
                                                        class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-700">Sedang
                                                        Belajar</span>
                                                @elseif($status === '-' || $status === null)
                                                    <span
                                                        class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">Tidak
                                                        diketahui</span>
                                                @else
                                                    <span
                                                        class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">{{ ucfirst($status) }}</span>
                                                @endif
                                            </div>
                                            <a href="{{ route('admin.courses.show', $enrollment->kursus->id) }}"
                                                class="ml-4 text-blue-600 hover:text-blue-800">
                                                <i class="fas fa-arrow-right"></i>
                                            </a>
                                        </div>
                                            ...
                                            <div class="flex-1">
                                                <div class="flex items-center gap-3 mb-2">
                                                    <h4 class="text-base font-semibold text-gray-800">
                                                        #{{ $transaction->transaction_code ?? $transaction->id }}</h4>
                                                    @if($transaction->status === 'paid')
                                                        <span
                                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-700">Berhasil</span>
                                                    @elseif($transaction->status === 'pending')
                                                        <span
                                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-700">Pending</span>
                                                    @elseif($transaction->status === 'expired')
                                                        <span
                                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">Expired</span>
                                                    @elseif($transaction->status === 'cancelled')
                                                        <span
                                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-700">Dibatalkan</span>
                                                    @elseif($transaction->status === 'refunded')
                                                        <span
                                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-purple-100 text-purple-700">Refund</span>
                                                    @else
                                                        <span
                                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">{{ ucfirst($transaction->status) }}</span>
                                                    @endif
                                                </div>
                                                <div class="flex items-center gap-4 text-sm text-gray-500 mb-2">
                                                    <span><i
                                                            class="fas fa-calendar mr-1"></i>{{ $transaction->created_at->format('d M Y H:i') }}</span>
                                                    <span><i class="fas fa-money-bill-wave mr-1"></i>Rp
                                                        {{ number_format($transaction->total_bayar, 0, ',', '.') }}</span>
                                                    @if($transaction->payment_method)
                                                        <span><i
                                                                class="fas fa-credit-card mr-1"></i>{{ ucfirst(str_replace('_', ' ', $transaction->payment_method)) }}</span>
                                                    @endif
                                                </div>
                                            </div>
                    ...
                    {{-- Sertifikat --}}
                    @php
                        $sertifikats = $user->enrollments->filter(fn($e) => $e->sertifikat)->map(fn($e) => $e->sertifikat);
                    @endphp
                    <div class="bg-white rounded-lg shadow-sm mt-6">
                        <div class="px-6 py-4 border-b border-gray-200">
                            <h3 class="text-lg font-bold text-gray-800">
                                <i class="fas fa-certificate text-yellow-500 mr-2"></i>Sertifikat
                            </h3>
                        </div>
                                        ...
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <div class="flex items-center gap-3 mb-2">
                                                    ...
                                                    <div>
                                                        <h4 class="text-base font-semibold text-gray-800">
                                                            {{ $sertifikat->enrollment->kursus->judul ?? 'Kursus' }}
                                                        </h4>
                                                        <p class="text-sm text-gray-500">
                                                            No: <span
                                                                class="font-mono font-medium">{{ $sertifikat->nomor_sertifikat ?? $sertifikat->kode_sertifikat ?? '-' }}</span>
                                                        </p>
                                                    </div>
                                                </div>
                                                <div class="flex items-center gap-4 text-sm text-gray-500">
                                                    @if($sertifikat->tanggal_terbit || $sertifikat->tanggal_diterbitkan)
                                                        <span><i class="fas fa-calendar mr-1"></i>Diterbitkan:
                                                            {{ ($sertifikat->tanggal_terbit ?? $sertifikat->tanggal_diterbitkan)->format('d M Y') }}</span>
                                                    @endif
                                                </div>
                                            </div>
