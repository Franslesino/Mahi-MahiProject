...
<body class="bg-gray-50" x-data="adminSidebar()" x-init="init()">
    ...
    <button @click="toggle()" class="lg:hidden fixed top-4 left-4 z-50 p-2 bg-white rounded-lg shadow-lg">
    ...
    <aside x-show="sidebarOpen" x-transition:enter="transform transition ease-out duration-300"
        ...
        <div class="flex-shrink-0 p-6 border-b border-gray-200">
            ...
            <div class="flex items-center gap-3">
                <div
                    class="w-14 h-14 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-full flex items-center justify-center flex-shrink-0">
                    <span
                        class="text-white text-xl font-bold">{{ strtoupper(substr(auth()->user()->name, 0, 2)) }}</span>
                </div>
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-2">
            ...
            @endphp
            <div x-data="{ open: {{ $isKursusSectionActive ? 'true' : 'false' }} }" class="space-y-1">
                <button type="button" @click="open = !open"
                    ...
                    :class="open ? 'bg-emerald-50 text-emerald-700' : ''" :aria-expanded="open"
                ...
                <div x-show="open" x-transition class="space-y-1 pl-10" id="menu-kursus">
                    ...
                    <a href="{{ route('admin.vouchers.index') }}"
                        class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition {{ request()->routeIs('admin.vouchers*') ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700 hover:bg-gray-50' }}">
    ...
    <main class="transition-all duration-300 min-h-screen" :class="sidebarOpen ? 'lg:ml-80' : 'lg:ml-0'">
        ...
        @if(session('success'))
            <div
                class="mx-8 mt-4 p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-lg flex items-center gap-3">
