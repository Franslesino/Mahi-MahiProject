                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition">
                        <div class="h-48 bg-gradient-to-br from-blue-500 to-blue-600 relative">
                            ...
                            <div class="absolute top-4 right-4">
                                <span
                                    class="px-3 py-1 rounded-full text-xs font-medium {{ $course->status === 'active' ? 'bg-green-100 text-green-700' : ($course->status === 'draft' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700') }}">
                            ...
                            @if($course->badge)
                                <div class="absolute top-4 left-4">
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-medium bg-{{ $course->badge_color }}-100 text-{{ $course->badge_color }}-700">
                                        {{ $course->badge }}
                                    </span>
                                </div>
                            @endif
                            ...
                            <div class="mb-2">
                                ...
                                @php
                                    ...
                                    $modeColors = [
                                        ...
                                        'Offline' => 'text-green-600 bg-green-50',
                                        'Hybrid' => 'text-purple-600 bg-purple-50',
                                    ];
                                    $modeClass = $modeColors[$mode] ?? 'text-purple-600 bg-purple-50';
                                @endphp
                                <span class="text-xs font-medium {{ $modeClass }} px-2 py-1 rounded ml-2">
                                    {{ $mode }}
                                </span>
                            </div>
                                ...
                                <div class="flex items-center gap-2 mb-4">
                                    @if($course->discount_price > 0)
                                        <span class="text-lg font-bold text-green-600">Rp
                                            {{ number_format($course->discount_price, 0, ',', '.') }}</span>
