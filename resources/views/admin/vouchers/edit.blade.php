...
<div class="p-8">
    ...
    <form action="{{ route('admin.vouchers.update', $voucher->id) }}" method="POST" class="bg-white rounded-lg shadow-sm p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" 
                          rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
                          placeholder="Deskripsi singkat tentang voucher ini...">{{ old('description', $voucher->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <p class="text-xs text-blue-600 mt-1">Saat ini: {{ $voucher->used_count }} kali digunakan</p>
                @endif
            </div>
            ...
            <div>
                ...
                <input type="date" 
                       name="start_date" 
                       value="{{ old('start_date', $voucher->start_date ? $voucher->start_date->format('Y-m-d') : date('Y-m-d')) }}"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
                       required>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2" id="coursesList">
                        ...
                        <label class="flex items-center gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer border border-transparent hover:border-emerald-200">
                            <input type="checkbox" 
                                   ...
                                   value="{{ $course->id }}"
                                   {{ in_array($course->id, old('allowed_courses', $voucher->allowed_courses ?? [])) ? 'checked' : '' }}
            ...
            <div class="md:col-span-2">
                <label class="flex items-center gap-2 cursor-pointer">
                    ...
                    <span class="text-sm font-medium text-gray-700">Aktifkan voucher</span>
                </label>
