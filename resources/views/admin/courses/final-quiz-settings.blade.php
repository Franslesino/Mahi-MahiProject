...
<div class="p-8">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="p-6">
            <form action="{{ route('admin.courses.final-quiz.update', $kursus->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div id="finalQuizSettings" style="display: {{ old('require_final_quiz', $kursus->require_final_quiz) ? 'block' : 'none' }};" class="space-y-6">
                    ...
                    <!-- Maksimal Percobaan -->
                    <div>
                        <label for="max_attempts" class="block text-sm font-bold text-gray-700 mb-2">
                            Maksimal Percobaan <span class="text-red-500">*</span>
                        </label>
                        <input type="number" id="max_attempts" name="max_attempts" 
                               min="1" max="10"
                               value="{{ old('max_attempts', $kursus->max_quiz_attempts ?? 3) }}"
                               required
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('max_attempts') border-red-500 @enderror">
                        @error('max_attempts')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <small class="text-gray-600 text-sm block mt-2">
                ...
                <div class="flex justify-end gap-3 mt-6 pt-6 border-t border-gray-200">
                    <a href="{{ route('admin.courses.show', $kursus->id) }}" 
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mt-6">
            ...
            <div class="p-6">
                ...
                <div class="mt-4 flex flex-wrap gap-3">
                    ...
                    <form action="{{ route('admin.courses.final-quiz.toggle-activation', $kursus->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" 
                    ...
                    <a href="{{ route('admin.courses.final-quiz.statistics', $kursus->id) }}" 
                            ...
                            <div class="border-2 border-gray-200 rounded-xl p-5 hover:border-purple-400 transition-all bg-gradient-to-r from-white to-gray-50">
                                <div class="flex items-start gap-4">
                                        ...
                                        <div class="flex items-center flex-wrap gap-2 mb-3">
                                            ...
                                            @if($question->questionBank)
                                                <span class="px-3 py-1 bg-blue-100 text-blue-700 text-xs font-semibold rounded-full">
                                                    <i class="fas fa-folder"></i> {{ $question->questionBank->name }}
                                                </span>
                                            @endif
                                        </div>
                                    ...
                                    <div class="flex-shrink-0">
                                        ...
                                        @else
                                            <form action="{{ route('admin.courses.final-quiz.remove-question', [$kursus->id, $question->id]) }}" 
                                                  method="POST" 
    ...
    @if($kursus->require_final_quiz && $kursus->final_quiz_id)
        <div id="importModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-4xl shadow-lg rounded-lg bg-white">
                ...
                <form action="{{ route('admin.courses.final-quiz.import-from-bank', $kursus->id) }}" method="POST">
                    @csrf
                        ...
                        <div id="importQuestionList" class="space-y-3 max-h-96 overflow-y-auto border border-gray-300 rounded-lg p-4">
                            @foreach($questionBanks as $bank)
                                @if($bank->questions->count() > 0)
                                    <div class="mb-4">
                                        <h4 class="font-semibold text-gray-700 mb-2 bg-gray-100 p-2 rounded">
                                            <i class="fas fa-folder"></i> {{ $bank->name }}
                                        </h4>
                                        @foreach($bank->questions as $question)
                                            <div class="import-question-item bg-white border border-gray-200 rounded-lg p-4 hover:border-blue-500 transition ml-4 mb-2">
                                                <div class="flex items-start gap-3">
                                                    ...
                                                    <label for="import_question_{{ $question->id }}" class="flex-1 cursor-pointer">
                                                        <div class="flex items-center gap-2 mb-2">
                                                            <span class="px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded-full font-semibold uppercase">
                                                                {{ str_replace('_', ' ', $question->type) }}
                                                            </span>
                                                            @if($question->points)
                                                        ...
                                                        <div class="text-gray-800 font-medium mb-2 import-question-text">
                                                            {!! nl2br(e(Str::limit($question->question_text, 150))) !!}
                                                        </div>
                            ...
                            @if($questionBanks->sum(function($bank) { return $bank->questions->count(); }) == 0)
    ...
    @if($kursus->require_final_quiz && $kursus->final_quiz_id)
        <div id="createQuestionModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div class="relative top-5 mx-auto p-6 border w-11/12 max-w-5xl shadow-lg rounded-lg bg-white my-10">
                ...
                <form action="{{ route('admin.courses.final-quiz.store-new-question', $kursus->id) }}" method="POST" id="createQuestionForm">
                    @csrf
                        ...
                        <div class="bg-blue-50 border-2 border-blue-200 rounded-lg p-5">
                            ...
                            <div id="bankSelectionContainer" class="hidden mt-4 pt-4 border-t border-blue-200">
                                ...
                                <select name="question_bank_id" id="question_bank_id"
                                    ...
                                    <option value="new">-- Buat Bank Baru: "Soal Final Quiz (Auto)" --</option>
                                    @foreach($questionBanks as $bank)
...
function selectAllImport() {
    document.querySelectorAll('#importQuestionList input[name="question_ids[]"]').forEach(cb => cb.checked = true);
}

function deselectAllImport() {
    document.querySelectorAll('#importQuestionList input[name="question_ids[]"]').forEach(cb => cb.checked = false);
}
...
function createOptionElement(text = '', isFixed = false) {
    ...
    const currentCounter = optionCounter;
    
    div.innerHTML = `
        <input type="radio" name="correct_option" value="${currentCounter}" required
               class="w-5 h-5 text-purple-600 border-gray-300 focus:ring-purple-500 cursor-pointer">
        <input type="text" name="options[${currentCounter}][text]" value="${text}" ${isFixed ? 'readonly' : ''} required
        ...
        <input type="hidden" name="options[${currentCounter}][is_correct]" value="0" class="is-correct-input">
        ${!isFixed ? `<button type="button" onclick="removeOption(${currentCounter})"
    ...
    radio.addEventListener('change', function() {
        ...
        document.querySelectorAll('.is-correct-input').forEach(input => input.value = '0');
        // Set this one to 1
        const hiddenInput = div.querySelector('.is-correct-input');
        hiddenInput.value = '1';
    });
