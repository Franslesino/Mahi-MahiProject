...
@section('content')
    @php
        ...
        foreach ($enrollments as $enrollment) {
            ...
            // Load materials through sections (how instructor stores them)
            $allMaterials = collect();
            if ($course->sections) {
                foreach ($course->sections as $section) {
                    if ($section->materials) {
                        $allMaterials = $allMaterials->merge($section->materials);
                    }
                }
            }
            // Also get materials directly linked to kursus_id (fallback)
            if ($course->materi) {
                $allMaterials = $allMaterials->merge($course->materi);
            }
            // Remove duplicates and filter non-draft
            $allMaterials = $allMaterials->unique('id')->filter(function ($m) {
                return ($m->status ?? 'published') !== 'draft';
            });

            $materiIds = $allMaterials->pluck('id')->toArray();
