...
@section('content')
        ...
        <div class="max-w-6xl mx-auto px-4">
            ...
            <div class="bg-white rounded-2xl shadow p-6 grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div class="lg:col-span-3">
                    <div class="flex items-center justify-between mb-4">
                        <div class="px-4 py-2 rounded-full bg-gray-100 text-sm font-semibold" id="question-counter">
                            1/{{ $assignment->questions->count() }}</div>

                        @if($assignment->time_limit || $assignment->duration_minutes)
                            <div class="px-4 py-2 rounded-full bg-red-100 text-red-700 text-sm font-semibold flex items-center gap-2"
                                id="timer-display">
                    ...
                    <div class="flex items-center justify-between mt-4 gap-3">
                        <button id="prev-btn"
                            class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition">Sebelumnya</button>
                        <button id="next-btn"
                            class="px-4 py-2 bg-emerald-700 text-white rounded-lg hover:bg-emerald-800 transition">Selanjutnya</button>
                    </div>
                </div>
    ...
    <form id="complete-form" action="{{ route('courses.materials.quiz.submit', [$course, $material->id]) }}" method="POST"
        class="hidden">
        @csrf
    ...
    <script>
        ...
        JSON_UNESCAPED_UNICODE
            ...
            // Timer configuration
            const timeLimitMinutes = {{ $assignment->time_limit ?? $assignment->duration_minutes ?? 0 }};
            ...
            function initTimer() {
                ...
                // Check if we have saved remaining time
                const savedTime = localStorage.getItem(timerStorageKey);
                if (savedTime !== null) {
                    remainingSeconds = parseInt(savedTime, 10);
                } else {
                    // First time starting quiz - set full time
                    remainingSeconds = timeLimitMinutes * 60;
                    localStorage.setItem(timerStorageKey, remainingSeconds);
                }
            ...
            function updateTimerDisplay() {
                ...
                // Change color when low on time (less than 5 minutes)
                if (timerDisplay) {
                    if (remainingSeconds <= 60) {
                        timerDisplay.className = 'px-4 py-2 rounded-full bg-red-600 text-white text-sm font-semibold flex items-center gap-2 animate-pulse';
                    } else if (remainingSeconds <= 300) {
                        timerDisplay.className = 'px-4 py-2 rounded-full bg-red-500 text-white text-sm font-semibold flex items-center gap-2';
                    }
                }
            }

            function autoSubmitQuiz() {
                alert('Waktu pengerjaan habis! Quiz akan otomatis diselesaikan.');
