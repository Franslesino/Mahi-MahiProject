    ...
    <div class="p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <a href="{{ route('admin.courses.index') }}"
                class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition focus:outline-none focus:ring-2 focus:ring-blue-300 block">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-green-600 mt-1 font-semibold">
                            <span
                                class="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-700 rounded-full"><i
                                    class="fas fa-arrow-up"></i> {{ $activeCourses }} aktif</span>
                        </p>
                    </div>
            ...
            <a href="{{ route('admin.users.index') }}"
                class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition focus:outline-none focus:ring-2 focus:ring-emerald-300 block">
                <div class="flex items-center justify-between">
                    <div>
            ...
            <a href="{{ route('admin.transactions.index') }}"
                class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition focus:outline-none focus:ring-2 focus:ring-purple-300 block">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-yellow-600 mt-1 font-semibold">
                            <span
                                class="inline-flex items-center gap-1 px-2 py-1 bg-yellow-50 text-yellow-700 rounded-full"><i
                                    class="fas fa-clock"></i> {{ $pendingTransactions }} pending</span>
                        </p>
                    </div>
            ...
            <!-- Total Pendapatan -->
            <a href="{{ route('admin.transactions.index') }}"
                class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition focus:outline-none focus:ring-2 focus:ring-amber-300 block">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-green-600 mt-1 font-semibold">
                            <span
                                class="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-700 rounded-full"><i
                                    class="fas fa-check-circle"></i> {{ $paidTransactions }} paid</span>
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        @foreach($categoryData as $index => $cat)
                            <div class="flex items-center gap-2">
                                <div class="w-3 h-3 rounded-full flex-shrink-0" style="background-color: {{ $colors[$index % 6] }}">
                                </div>
                ...
                @if($monthlyData->sum() > 0)
                    <div class="flex items-center gap-2 mb-4 chart-toggle">
                        <div class="flex rounded-lg border border-gray-200 overflow-hidden text-xs">
                            <button id="chartTypeLine" class="px-3 py-1.5 bg-gray-100 text-gray-800 font-semibold"
                                aria-pressed="true" aria-label="Tampilkan grafik garis">Line</button>
                            <button id="chartTypeBar" class="px-3 py-1.5 text-gray-600 hover:bg-gray-100" aria-pressed="false"
                                aria-label="Tampilkan grafik batang">Bar</button>
                        </div>
                    </div>
                        ...
                        @foreach($genderData as $index => $gender)
                            <div class="flex items-center gap-2">
                                <div class="w-3 h-3 rounded-full flex-shrink-0"
                                    style="background-color: {{ $genderColors[$index % 3] }}"></div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs text-gray-500">{{ $gender['gender'] }}</p>
                                    <p class="text-sm font-bold text-gray-800">{{ $gender['count'] }} ({{ $gender['percentage'] }}%)
                                    </p>
                                </div>
                            </div>
                        @endforeach
                        ...
                        @foreach($topCourses as $index => $course)
                            <div class="flex items-center justify-between group hover:bg-gray-50 p-3 rounded-lg transition">
                                <div class="flex items-center gap-3 flex-1">
                                    <div
                                        class="w-8 h-8 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-lg flex items-center justify-center flex-shrink-0">
                                        <span class="text-white font-bold text-sm">{{ $index + 1 }}</span>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p
                                            class="text-sm font-semibold text-gray-800 truncate group-hover:text-emerald-600 transition">
                                            {{ $course['title'] }}
                                        </p>
                ...
                @if($recentEnrollments->count() > 0)
                    <div class="space-y-1">
                        @foreach($recentEnrollments as $enrollment)
                            <div
                                class="flex items-center justify-between py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 px-3 -mx-3 rounded-lg transition">
                                <div class="flex items-center gap-3">
                                    <div
                                        ...
                                        <span
                                            class="text-white font-bold text-sm">{{ substr($enrollment->user->name, 0, 1) }}</span>
                                    </div>
                                    <div>
        ...
        <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
            <a href="{{ route('admin.courses.create') }}"
                class="bg-white border-2 border-emerald-500 hover:bg-emerald-50 rounded-lg p-4 flex items-center gap-3 transition">
            ...
            <a href="{{ route('admin.users.create') }}"
                class="bg-white border-2 border-blue-500 hover:bg-blue-50 rounded-lg p-4 flex items-center gap-3 transition">
            ...
            <a href="{{ route('admin.transactions.index') }}"
                class="bg-white border-2 border-purple-500 hover:bg-purple-50 rounded-lg p-4 flex items-center gap-3 transition">
