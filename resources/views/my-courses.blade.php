...
@section('content')
    @php
        ...
        foreach ($enrollments as $enrollment) {
            ...
            // Load sections.materials jika belum, sama seperti getMaterialStats
            $course->loadMissing([
            ...
            // Ambil materi dari sections.materials
            ...
            // Hitung total dan completed (materi + quiz + final quiz jika required)
            $totalItems = $materiCount + $quizCount + ($finalQuizRequired ? 1 : 0);
            $completedItems = $completedMaterialCount + $completedQuizCount + (($finalQuizRequired && $finalQuizPassed) ? 1 : 0);
            $progress = $totalItems > 0 ? round(($completedItems / $totalItems) * 100) : 0;

            $materialsCompleted = $materiCount > 0 ? $completedMaterialCount >= $materiCount : true;
            $quizzesCompleted = $quizCount > 0 ? $completedQuizCount >= $quizCount : true;

            // isCompleted: kursus harus punya minimal 1 item DAN semua selesai
            // Kursus dengan 0/0 materi dianggap BELUM selesai (ongoing)
            $hasContent = $totalItems > 0;
            $allLearningDone = $materialsCompleted && $quizzesCompleted;
            $isCompleted = $hasContent && $allLearningDone && (!$finalQuizRequired || $finalQuizPassed);
            ...
            $courseProgressMap[$course->id] = [
                ...
                'finalQuizRequired' => $finalQuizRequired,
    ...
    <div class="bg-gray-50 min-h-screen">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div class="flex gap-3 mb-6 overflow-x-auto pb-2">
                <button
                    class="filter-tab active px-6 py-2 bg-teal-700 text-white rounded-full font-semibold whitespace-nowrap transition hover:bg-teal-800"
                    data-filter="ongoing">
                    Berlangsung <span class="ml-1 px-2 py-0.5 bg-white/20 rounded-full text-xs">{{ $ongoingCount }}</span>
                </button>
                <button
                    class="filter-tab px-6 py-2 bg-white text-gray-700 rounded-full font-semibold whitespace-nowrap border border-gray-300 transition hover:bg-gray-50"
                    data-filter="completed">
                    Selesai <span class="ml-1 px-2 py-0.5 bg-gray-200 rounded-full text-xs">{{ $completedCount }}</span>
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <div
                    class="bg-gradient-to-r from-emerald-500 to-teal-500 rounded-2xl p-4 text-white shadow-sm flex items-center gap-3">
                ...
                <div
                    class="bg-gradient-to-r from-blue-500 to-indigo-500 rounded-2xl p-4 text-white shadow-sm flex items-center gap-3">
                    <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
                        <i class="fas fa-certificate text-xl"></i>
                    </div>
                    <div>
                        <div class="text-2xl font-bold">{{ $certificateEligibleCount }}</div>
                        <div class="text-sm opacity-90">Sertifikat Tersedia</div>
                    </div>
                </div>
            </div>

            @if($enrollments->isEmpty() && (!isset($pendingTransactions) || $pendingTransactions->isEmpty()))
                <!-- Empty State -->
            ...
            @else
                {{-- Section: Pending Payment Transactions --}}
                @if(isset($pendingTransactions) && $pendingTransactions->count() > 0)
                    <div class="mb-6">
                        <h2 class="text-lg font-bold text-gray-900 mb-3 flex items-center">
                            ...
                            Menunggu Pembayaran
                            <span class="ml-2 px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full text-xs font-semibold">
                        ...
                        <div class="space-y-4">
                            @foreach($pendingTransactions as $transaction)
                                ...
                                @endphp
                                <div
                                    class="bg-gradient-to-r from-amber-50 to-orange-50 border-l-4 border-amber-400 rounded-2xl shadow-sm p-4">
                                    <div class="flex gap-4">
                                        <!-- Course Thumbnail -->
                                        <div class="flex-shrink-0">
                                            <div
                                                class="w-24 h-24 sm:w-28 sm:h-28 rounded-xl overflow-hidden bg-gradient-to-br from-gray-700 to-gray-900">
                                                @if($course->image_url)
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="mb-2">
                                                <span
                                                    class="inline-block px-3 py-1 text-xs font-bold rounded-full bg-amber-100 text-amber-600">
                                                    Menunggu Pembayaran
                                                </span>
                                            </div>
                                            ...
                                            <p class="text-sm text-gray-600 mb-2">
                                                Total: <span class="font-bold text-gray-900">Rp
                                                    {{ number_format($transaction->total_bayar, 0, ',', '.') }}</span>
                                            </p>
                                            <p class="text-xs text-amber-700 mb-3">
                                                <i class="fas fa-clock mr-1"></i>
                                                Bayar sebelum:
                                                {{ $transaction->formatted_deadline ?? $transaction->payment_deadline->format('d M Y, H:i') }}
                                            </p>

                                            <!-- Action Buttons -->
                                            <div class="flex flex-wrap gap-2">
                                                <a href="{{ route('transactions.show', $transaction) }}"
                                                    class="inline-flex items-center px-4 py-2 bg-amber-500 text-white rounded-lg text-sm font-semibold hover:bg-amber-600 transition shadow-sm">
                                                    <i class="fas fa-credit-card mr-2"></i>
                                                    Lanjutkan Pembayaran
                                                </a>
                                                <a href="{{ route('courses.show', $course) }}"
                                                    class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-50 transition">
                                        ...
                                        <div class="flex items-start">
                                            <span
                                                class="px-3 py-1 bg-amber-500 text-white rounded-full text-xs font-semibold shadow-md">
                ...
                <div class="space-y-4" id="coursesList">
                    @foreach($enrollments as $enrollment)
                        ...
                        <div class="course-item bg-white rounded-2xl shadow-sm hover:shadow-md transition p-4"
                            data-status="{{ $isCompleted ? 'completed' : 'ongoing' }}" data-course-name="{{ strtolower($judul) }}">
                            ...
                            <div class="flex gap-4">
                                ...
                                <div class="flex-shrink-0">
                                    <div
                                        class="w-24 h-24 sm:w-28 sm:h-28 rounded-xl overflow-hidden bg-gradient-to-br from-gray-700 to-gray-900">
                                        @if($course->image_url)
                                            <img src="{{ $course->image_url }}" alt="{{ $judul }}" class="w-full h-full object-cover">
                                        @else
                                            <div class="w-full h-full flex items-center justify-center">
                                                <i class="fas fa-graduation-cap text-white text-3xl opacity-50"></i>
                                            </div>
                                        ...
                                        <div class="space-y-2">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="text-xs text-gray-500">
                                                    {{ $progressData['completedCount'] }}/{{ $progressData['materiCount'] }} Materi
                                                </span>
                                            </div>
                                            ...
                                            <div class="flex flex-wrap items-center gap-2">
                                                @if($certificate)
                                                    ...
                                                    <div class="flex items-center gap-2 flex-wrap">
                                                        <button onclick="viewCertificate({{ $enrollment->id }})"
                                                        ...
                                                        <a href="{{ route('student.certificate.download', $enrollment) }}"
                                                ...
                                                @else
                                                    ...
                                                    <div
                                                        class="flex items-center gap-2 text-sm text-amber-600 bg-amber-50 px-3 py-2 rounded-lg">
                                    ...
                                    @else
                                        ...
                                        <div class="space-y-2">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <span class="text-sm font-bold text-gray-700 min-w-[45px]">{{ $progress }}%</span>
                                            </div>
                                            {{-- Indikator terpisah untuk Materi dan Quiz --}}
                                            <div class="flex flex-wrap gap-3 text-xs">
                                                @if($progressData['materiCount'] > 0)
                                                    <span
                                                        class="inline-flex items-center gap-1 {{ $progressData['materialsCompleted'] ? 'text-emerald-600' : 'text-gray-500' }}">
                                                        <i
                                                            class="fas {{ $progressData['materialsCompleted'] ? 'fa-check-circle' : 'fa-book' }}"></i>
                                                        Materi: {{ $progressData['completedCount'] }}/{{ $progressData['materiCount'] }}
                                                    </span>
                                                @endif
                                                @if($progressData['quizCount'] > 0)
                                                    <span
                                                        class="inline-flex items-center gap-1 {{ $progressData['quizzesCompleted'] ? 'text-emerald-600' : 'text-gray-500' }}">
                                                        <i
                                                            class="fas {{ $progressData['quizzesCompleted'] ? 'fa-check-circle' : 'fa-question-circle' }}"></i>
                                                        Quiz: {{ $progressData['completedQuizCount'] }}/{{ $progressData['quizCount'] }}
                                                    </span>
                                                @endif
                                                @if($finalQuizRequired)
                                                    <span
                                                        class="inline-flex items-center gap-1 {{ $finalQuizPassed ? 'text-emerald-600' : 'text-amber-600' }}">
                                                        <i class="fas {{ $finalQuizPassed ? 'fa-check-circle' : 'fa-flag' }}"></i>
                                                        Final Quiz: {{ $finalQuizPassed ? 'Lulus' : 'Belum' }}
                                                    </span>
                                                @endif
                                            </div>

                                            @if($finalQuizRequired && !$finalQuizPassed && $materialsCompleted && $progressData['quizzesCompleted'])
                                            ...
                                            <div class="flex flex-wrap gap-2 pt-1">
                                                <a href="{{ route('student.course.learn', $course) }}"
                                                    ...
                                                    {{ $finalQuizRequired && $materialsCompleted ? 'Kerjakan Final Quiz' : 'Lanjutkan Belajar' }}
                                                </a>
                                                @if($finalQuizRequired)
                                                    @if($finalQuizPassed)
                                                        <span
                                                            class="inline-flex items-center gap-2 px-3 py-2 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-semibold">
                                                            <i class="fas fa-check-circle"></i> Final quiz lulus
                                                        </span>
                                                    @elseif($materialsCompleted)
                                                        {{-- Materials completed, can take final quiz --}}
                                                        <a href="{{ route('courses.final-quiz.show', $course->id) }}"
                                                        ...
                                                        @if(!is_null($finalQuizScore))
                                                            <span
                                                                class="inline-flex items-center gap-2 px-3 py-2 bg-amber-50 text-amber-700 rounded-lg text-xs font-semibold">
                                                                <i class="fas fa-chart-line"></i> Nilai terakhir:
                                                                {{ number_format($finalQuizScore, 2) }}%
                                                            </span>
                                                        @endif
                                                    @else
                                                        {{-- Materials not completed, show locked button --}}
                                                        <span
                                                            class="inline-flex items-center gap-2 px-4 py-2 bg-gray-300 text-gray-500 rounded-lg text-sm font-semibold cursor-not-allowed"
                                                            title="Selesaikan semua materi terlebih dahulu">
                                                            <i class="fas fa-lock"></i> Final Quiz
                                                        </span>
                                                        <span class="text-xs text-gray-500">
                                                            Selesaikan {{ $progressData['materiCount'] - $progressData['completedCount'] }}
                                                            materi lagi
                                                        </span>
                                                    @endif
                                                @endif
    ...
    <div id="courseCompleteOverlay" class="hidden fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center relative overflow-hidden">
            ...
            <div class="relative z-10 space-y-4">
                <div
                    class="w-20 h-20 mx-auto bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl flex items-center justify-center shadow-lg">
                ...
                <div class="flex flex-col gap-3">
                    <a id="downloadCertificateBtn" href="#"
                        class="hidden px-4 py-3 bg-emerald-600 text-white rounded-xl font-semibold hover:bg-emerald-700 transition">
                        Unduh Sertifikat
                    </a>
                    <button id="closeCompleteModal"
                        class="px-4 py-3 bg-gray-100 text-gray-800 rounded-xl font-semibold hover:bg-gray-200 transition">
                        Kembali
                    </button>
                </div>
            </div>
