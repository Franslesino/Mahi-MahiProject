...
<body class="bg-gray-50" x-data="{ sidebarOpen: true }">
    ...
    <button @click="sidebarOpen = !sidebarOpen"
        ...
        <svg class="w-5 h-5 text-gray-600 transition-transform duration-300" :class="sidebarOpen ? '' : 'rotate-180'"
...
$sidebarInitial = strtoupper(substr($sidebarUser->first_name ?? $sidebarUser->name, 0, 1));
    ...
    : 'data:image/svg+xml;utf8,' . rawurlencode(
        ...
        '</text>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <span
                        class="inline-block mt-1 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">
                        Instruktur
                    </span>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative" x-data="{ openNotif: false }" @keydown.escape.window="openNotif=false">
                        <button @click="openNotif = !openNotif"
                            ...
                            <span id="notif-badge"
                                class="hidden absolute -top-1 -right-1 min-w-[18px] h-4 px-1 bg-red-500 text-white text-[11px] rounded-full flex items-center justify-center"></span>
                        </button>
                        <div x-show="openNotif" x-transition @click.away="openNotif=false; markAllAsReadOnClose()"
                            ...
                            <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-2">
                                    <form action="{{ route('instructor.notifications.mark-all-read') }}" method="POST"
                                        id="notif-mark-all-form" class="hidden">
                                        @csrf
                                        <button type="submit"
                                            class="text-xs text-blue-600 hover:text-blue-800 font-semibold">Tandai
                                            semua</button>
                                    </form>
                                    <button onclick="deleteAllNotifications()" id="notif-delete-all-btn"
    ...
    : 'data:image/svg+xml;utf8,' . rawurlencode(
        ...
        '</text>
                                ...
                                }}" alt="Avatar" class="w-8 h-8 rounded-full object-cover ring-1 ring-blue-500/20">
                    </div>
                </div>
            </div>
        </header>
        ...
        @if(session('success'))
            <div
                class="mx-8 mt-4 p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-lg flex items-center justify-between">
                <div class="flex items-center gap-3">
        ...
        @if(session('error'))
            <div
                class="mx-8 mt-4 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg flex items-center justify-between">
                <div class="flex items-center gap-3">
        ...
        @if(session('info'))
            <div
                class="mx-8 mt-4 p-4 bg-blue-50 border border-blue-200 text-blue-700 rounded-lg flex items-center justify-between">
                <div class="flex items-center gap-3">
