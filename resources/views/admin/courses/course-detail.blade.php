                    ...
                    <div class="section-container" data-section-id="{{ $section->id }}">
                        ...
                        <div class="p-4 hover:bg-gray-50 flex items-center justify-between">
                            <div class="flex items-center gap-3 flex-1">
                                ...
                                <button class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-gray-600 transition" onclick="toggleSection({{ $section->id }})">
    ...
    if (sortableContainer) {
        new Sortable(sortableContainer, {
            ...
            onEnd: function(evt) {
                ...
                @else
                const reorderUrl = "{{ route('instructor.courses.sections.reorder', $course) }}";
                @endif
    ...
    document.querySelectorAll('.sortable-materials').forEach(container => {
        new Sortable(container, {
            ...
            onEnd: function(evt) {
                ...
                // Save new order via AJAX
                @if($isAdmin)
                const reorderUrl = "{{ route('admin.courses.modules.materials.reorder', [$course, '__SECTION__']) }}".replace('__SECTION__', sectionId);
                @else
                const reorderUrl = "{{ route('instructor.courses.materials.reorder', $course) }}";
                @endif
                ...
                .catch(error => {
                    ...
                    Swal.fire({
                        ...
                        position: 'top-end',
                        icon: 'error',
                        title: 'Gagal menyimpan urutan materi',
                        showConfirmButton: false,
                        timer: 2000
                    });
                });
